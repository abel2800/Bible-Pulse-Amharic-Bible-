...
class ReadingPlanProvider extends ChangeNotifier {
  ...
  Map<String, List<DailyReading>> _dailyReadings = {};
  ...
  List<ReadingPlan> get completedPlans {
    final completedPlanIds = _userPlans
        .where((up) => up.isCompleted)
        .map((up) => up.planId)
        .toSet();
    return _availablePlans.where((p) => completedPlanIds.contains(p.id)).toList();
  }
  ...
  Future<void> loadPlans() async {
    ...
    try {
      final String plansJson = await rootBundle.loadString('assets/plans/reading_plans.json');
      final List<dynamic> plansData = json.decode(plansJson);
      _availablePlans = plansData.map((data) => ReadingPlan.fromJson(data)).toList();
  ...
  List<ReadingPlan> searchPlans(String query) {
    ...
    return _availablePlans.where((plan) {
      return plan.name.toLowerCase().contains(lowerQuery) ||
          plan.title.values.any((title) => title.toLowerCase().contains(lowerQuery)) ||
